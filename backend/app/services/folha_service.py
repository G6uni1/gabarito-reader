...
from app.vision.layout_folha import (
    LARGURA, ALTURA, ALTERNATIVAS_X,
    RAIO_BOLINHA, calcular_layout
)
...
def gerar_folha_aluno(
    ...
    """Gera a folha de resposta personalizada com layout dinâmico."""
    ...
    cv2.putText(folha, "Nº", (55, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.4, 0, 1)
    for letra, x in ALTERNATIVAS_X.items():
        cv2.putText(folha, letra, (x - 4, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.4, 0, 1)

    # ─── Questões (layout dinâmico) ──────────────────────────────────────────
